...
if (isset($_POST["filterValue"])) {
    ...
    switch ($filter) {
        case "needrepair":
            ...
            WHERE school_inventory.item_status = 'Need Repair' AND school_inventory.is_visible = TRUE";
            break;
        case "condemned":
            $filterQuery = "SELECT high_schools.school_name, school_inventory.item_code, school_inventory.item_article, school_inventory.item_status, school_inventory.item_date_acquired
            FROM school_inventory 
            JOIN high_schools ON school_inventory.school_id = high_schools.school_id
            WHERE school_inventory.item_status = 'Condemned' AND school_inventory.is_visible = TRUE";
            break;
        case "all":
            $filterQuery = "SELECT high_schools.school_name, school_inventory.item_code, school_inventory.item_article, school_inventory.item_status, school_inventory.item_date_acquired
            FROM school_inventory 
            JOIN high_schools ON school_inventory.school_id = high_schools.school_id
            WHERE (school_inventory.item_status = 'Need Repair' OR school_inventory.item_status = 'Condemned') AND school_inventory.is_visible = TRUE";
            break;
        default:
